...
func newReleaseBuildCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: "  language-tools release build --version v1.2.3 --output-dir dist",
		RunE:    runReleaseBuild,
	}
	cmd.Flags().String("module-dir", ".", "Go module root to build")
	cmd.Flags().String("output-dir", "dist", "directory archives and checksums.txt are written to")
	cmd.Flags().String("binary-name", "language-tools", "binary base name embedded in each archive")
	cmd.Flags().String("version", "dev", "version tag embedded in each archive's filename")
